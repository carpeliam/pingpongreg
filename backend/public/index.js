...
/***/ function(module, exports, __webpack_require__) {
	...
	var _actioncable = __webpack_require__(202);
	...
	window.cable = _actioncable2.default.createConsumer();
	window.cable.subscriptions.create('TableChannel', {
	  ...
	  disconnected: function disconnected() {
	    ...
	    return console.log('disconnected', args);
	  },
	  rejected: function rejected() {
	    ...
	    return console.log('rejected', args);
	  },
	  received: function received(data) {
	    return store.dispatch(dataReceivedAction(data));
	  }
	});
...
/***/ function(module, exports) {
	...
	var ENQUEUE_FOR_TABLE = exports.ENQUEUE_FOR_TABLE = 'ENQUEUE_FOR_TABLE';
	var REMOVE_RESERVATION = exports.REMOVE_RESERVATION = 'REMOVE_RESERVATION';
	...
	function enqueuePlayer(player) {
	  return { type: ENQUEUE_FOR_TABLE, player: player };
	}
	
	function fetchTables() {
	  return function (dispatch) {
	    return fetch('/table_queues').then(function (response) {
	...
	function reserveTable(tableId) {
	  ...
	  return function (dispatch) {
	    return fetch('/table_queues/' + tableId + '/table_queue_entries', { method: 'POST' }).then(function (response) {
	      return response.json();
	    }).then(function (json) {
	      return dispatch(enqueuePlayer(json));
	    });
	  };
	}
	...
	function removeReservation(reservation) {
	  return function (dispatch) {
	    return fetch('/table_queue_entries/' + reservation.id, { method: 'DELETE' }).then(function () {
	      return dispatch({ type: REMOVE_RESERVATION, reservation: reservation });
	    });
	  };
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	function tables() {
	  ...
	  switch (action.type) {
	    ...
	    case _actions.ENQUEUE_FOR_TABLE:
	      {
	        return list.update(list.findIndex(function (table) {
	          return table.id === action.player.table_queue_id;
	        }), function (table) {
	          return table.updateIn(['queue'], function (queue) {
	            return queue.push(action.player);
	          });
	        }).toJS();
	      }
	    case _actions.REMOVE_RESERVATION:
	      {
	        return list.update(list.findIndex(function (table) {
	          return table.id === action.reservation.table_queue_id;
	        }), function (table) {
	          return table.updateIn(['queue'], function (queue) {
	            return queue.filterNot(function (r) {
	              return r.get('id') === action.reservation.id;
	            });
	          });
	        }).toJS();
	      }
